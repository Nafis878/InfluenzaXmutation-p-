...
# ══════════════════════════════════════════════════════════════════════════════
# SIMULATION SCOPE DECLARATION
# ══════════════════════════════════════════════════════════════════════════════
SIMULATION_DISCLAIMER = """
╔══════════════════════════════════════════════════════════════════════════════╗
║  SIMULATION STUDY — SCOPE AND VALIDITY DECLARATION                          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  This module is a PROOF-OF-CONCEPT SIMULATION, not an empirical study.      ║
║                                                                              ║
║  What this demonstrates:                                                     ║
║    (1) The statistical methods (chi-square, regression, ARIMA, transformer)  ║
║        correctly detect drift in data that follows sigmoid-onset dynamics.   ║
║    (2) These same methods are validated on REAL biological data in the       ║
║        proxy_validate_on_influenza() section below (positive control:        ║
║        H1N1 post-2009 pandemic drift; negative control: pre-drift period).   ║
║    (3) Under the formal isomorphism conditions C1–C4 (see                    ║
║        dual_drift_framework.py), the same methods would apply to real LLM    ║
║        behavioral monitoring data meeting those conditions.                  ║
║                                                                              ║
║  What this CANNOT claim:                                                     ║
║    - That real LLM behavioral drift follows sigmoid dynamics.                ║
║    - That the chi-square enrichment result is empirically significant        ║
║      (the critical region is defined from the SAME simulation that           ║
║       injects drift into it — this violates isomorphism condition C3).       ║
║    - AUC/F1 metrics from this simulation are theoretical performance bounds  ║
║      under ideal conditions, NOT measured values on real deployment data.    ║
║                                                                              ║
║  Required for a valid empirical paper:                                       ║
║    Real LLM behavioral logs with a pre-specified alignment checklist (C3)    ║
║    and ≥8 monitored versions (C4). See dual_drift_framework.py for details.  ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""


# ══════════════════════════════════════════════════════════════════════════════
# PROXY VALIDATION — Real Biological Drift as Method Proof-of-Concept
# Uses ACTUAL influenza divergence data to validate detection methods on a
# known positive control (H1N1 post-2009 drift) and negative control
# (pre-drift stable period). This is the empirical grounding for the framework.
# ══════════════════════════════════════════════════════════════════════════════
def proxy_validate_on_influenza():
    """
    Validate detection methods on REAL biological drift data before simulation.
    Uses phase1_h1n1_divergence_rates.csv from the influenza pipeline.

    Positive control  : H1N1 2009–2017 (known pandemic drift — should detect)
    Negative control  : pre-2009 stable period (should NOT trigger false alarm)
    """
    from scipy.stats import chi2_contingency, linregress
    from pathlib import Path

    influenza_csv = Path('C:/Users/UseR/outputs/phase1_h1n1_divergence_rates.csv')
    if not influenza_csv.exists():
        print('  [proxy validation] phase1_h1n1_divergence_rates.csv not found — skipping.')
        print('  Run mutation_analysis.py first to generate the required file.')
        return None

    dr = pd.read_csv(influenza_csv)
    dr['Year'] = dr['Year'].astype(int)
    pan = dr.dropna(subset=['pandemic_mean']).copy()
    pan = pan[pan['Year'] >= 2009].sort_values('Year')

    if len(pan) < 4:
        print('  [proxy validation] Insufficient pandemic lineage years — skipping.')
        return None

    # ── Positive control: detect drift in pandemic lineage ──
    t = (pan['Year'].values - 2009).astype(float)
    d = pan['pandemic_mean'].values
    w = pan['pandemic_n'].values.astype(float)
    slope = float(np.sum(w * t * d) / np.sum(w * t**2))
    slope_se = float(np.std(d - slope * t) / (np.sqrt(np.sum(t**2)) + 1e-9))
    t_stat = slope / (slope_se + 1e-9)
    p_slope = float(2 * (1 - __import__('scipy').stats.t.cdf(abs(t_stat), df=len(t)-1)))

    # ── Negative control: pre-drift flat period ──
    pre = dr[dr['Year'] < 2009].dropna(subset=['mean_distance'])
    neg_slope, neg_intercept, neg_r, neg_p, _ = linregress(
        pre['Year'].values, pre['mean_distance'].values) if len(pre) >= 3 else (0, 0, 0, 1, 0)

    # ── Chi-square: high-drift vs low-drift years ──
    median_d = np.median(d)
    high_drift = (d > median_d).astype(int)
    late_years = (t > t.max() / 2).astype(int)
    table = np.array([
        [(high_drift & late_years).sum(),      (high_drift & (1-late_years)).sum()],
        [((1-high_drift) & late_years).sum(),  ((1-high_drift) & (1-late_years)).sum()]
    ])
    chi2, p_chi, _, _ = chi2_contingency(table)
    enrich = (table[0,0]/(table[0].sum()+1e-9)) / (table[1,0]/(table[1].sum()+1e-9) + 1e-9)

    pos_ctrl_pass = (slope > 0.5 and p_slope < 0.05)
    neg_ctrl_pass = (abs(neg_slope) < 0.5)
    chi_pass      = (p_chi < 0.05 and enrich > 1.2)

    result = {
        'positive_control': {
            'description': 'H1N1 pandemic lineage drift rate (2009-2017)',
            'slope_aa_per_yr': round(slope, 4),
            'p_value': round(p_slope, 4),
            'PASS': pos_ctrl_pass,
        },
        'negative_control': {
            'description': 'Pre-pandemic H1N1 (stable period)',
            'slope_aa_per_yr': round(float(neg_slope), 4),
            'p_value': round(float(neg_p), 4),
            'PASS': neg_ctrl_pass,
        },
        'chi_square_on_real_data': {
            'enrichment_ratio': round(float(enrich), 3),
            'chi2': round(float(chi2), 3),
            'p_value': round(float(p_chi), 4),
            'PASS': chi_pass,
        },
        'framework_validated': pos_ctrl_pass and neg_ctrl_pass,
    }

    (DIRS['rep'] / 'proxy_validation_influenza.json').write_text(
        json.dumps(result, indent=2), encoding='utf-8')

    print(f'\n  ── Proxy Validation on Real Influenza Data ──────────────────')
    print(f'  Positive control (H1N1 drift 2009-2017):')
    print(f'    slope = {slope:.4f} aa/yr  p={p_slope:.4f}  '
          f'{"✓ PASS" if pos_ctrl_pass else "✗ FAIL"}')
    print(f'  Negative control (pre-pandemic stable):')
    print(f'    slope = {neg_slope:.4f} aa/yr  p={neg_p:.4f}  '
          f'{"✓ PASS" if neg_ctrl_pass else "✗ FAIL"}')
    print(f'  Chi-square on real data:')
    print(f'    enrichment={enrich:.3f}  chi2={chi2:.3f}  p={p_chi:.4f}  '
          f'{"✓ PASS" if chi_pass else "✗ FAIL"}')
    print(f'  Framework validated on real biological data: '
          f'{"YES" if result["framework_validated"] else "NO — check data"}')
    print(f'  ─────────────────────────────────────────────────────────────')
    return result
...
def generate_agent_data():
    """
    Simulate 30 agent versions (v1.0–v5.0 in 0.2 steps).
    Drift is injected after v2.6, accelerating after v3.8.
    Critical function changes cluster in drift region.

    IMPORTANT — CIRCULAR VALIDATION RISK:
    The variable `in_critical_prob` is set proportional to `drift_factor`,
    meaning critical function co-occurrence IS the injected drift signal.
    Chi-square enrichment on this data detects what was programmed.
    This is valid for a simulation study showing theoretical detection bounds,
    but NOT as empirical evidence that the methods detect real-world LLM drift.
    The proxy_validate_on_influenza() function provides real-data grounding.
    """
    np.random.seed(42)
    n_versions = 30
...
def write_markdown_report(df, m1, m3, m6_out, ens_out, alert):
    ...
    lines = [
        ...
        '',
        '> **SIMULATION STUDY NOTICE**  ',
        '> All results in this report are derived from a **calibrated simulation**,',
        '> not from real LLM deployment data. The detection methods are validated on',
        '> real biological drift data (influenza H1N1 post-2009) via',
        '> `proxy_validate_on_influenza()`. The chi-square enrichment result on',
        '> simulated data is a **theoretical performance bound** under ideal conditions,',
        '> not an empirical measurement. See `dual_drift_framework.py` for the formal',
        '> conditions under which these methods transfer to real LLM behavioral data.',
        '',
        '## Executive Summary',
        f'Analysis of {n_total} **simulated** agent versions '
        f'(v{df["version"].min()}–v{df["version"].max()}) '
        f'detected **{n_drift} drift events** ({n_drift/n_total*100:.0f}% of versions).',
        f'Under simulation assumptions, drift is enriched in critical functions '
        f'(**{m1["enrichment"]:.2f}× enrichment**, p={m1["p"]:.4f}).',
        f'Current simulated system status: **{alert["status"]}** '
        f'(ensemble score={alert["drift_score"]:.3f}).',
        '',
        '## Pipeline Overview',
        '',
...
if __name__ == '__main__':
    ...
    # ── Proxy Validation: methods tested on REAL biological drift first ─────
    print('── Proxy Validation on Real Influenza Data ───────────────')
    proxy_result = proxy_validate_on_influenza()

    # Part 1
    print('\n── Part 1: Simulation Data Preparation ───────────────────')
